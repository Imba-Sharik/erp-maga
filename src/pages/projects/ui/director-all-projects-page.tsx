    ...
    <div className="@container flex min-h-0 flex-1 flex-col gap-6">
      <header className="flex shrink-0 flex-wrap items-start justify-between gap-4">
        <div className="flex flex-col gap-1.5">
          <h1 className="font-heading text-foreground font-bold">Все проекты</h1>
          <p className="text-muted-foreground hidden max-w-[640px] text-sm md:block">
            Все проекты, проходящие через MAG
          </p>
        </div>
        <Button
          type="button"
          className="bg-primary text-primary-foreground hover:bg-primary/90 hidden h-10 rounded-[10px] px-4 @3xl:inline-flex"
